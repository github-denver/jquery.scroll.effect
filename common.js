var UI = (function () {
  var fullPageScroll = {
    initialize: function () {
      ...
      function _activate() {
        $(startScreen).find(selector).each(function (index, element) {
      ...
      function _initialize() {
        ...
        if (typeof sideNavigation === 'undefined' && nextContentButton) {
          ...
          // console.log("nextContentButton: ", nextContentButton);
          $(nextContentButton).off('click').on('click', function (event) {
      ...
      $(window).on('scroll', function () {
        ...
        _clearTimeout = setTimeout(function () {
          ...
          function _mouseWheelUp() {
            ...
            if (reverse && $(element).offset().top - distance > _scrollTop) { // element의 상단 부분이 보일 때..
  ...
  var sideNavigation = {
    ...
    buttonAnimation: function () {
      ...
      $(_button).off('click').on('click', function (event) {
        event.preventDefault();
    ...
    scrollAnimation: function () {
      ...
      if ($(window).height() >= this.tempScreenHeight && !this.isFalse && !this.isResize) {
        for (i = 0; i < _length; i++) {
          if (_scrollTop >= this.offsetArray[i] && _scrollTop <= this.offsetArray[i + 1]) { // 2020-10-05 수정
            /* 2020-10-05 수정 시작 */
          ...
          if (_scrollTop > this.offsetArray[_length - 1]) {
            _index = _length - 1;
            // console.log("[sideNavigation{} scrollAnimation()] 마지막 화면입니다. _index: ", _index);
          }
    ...
    initialize: function () {
      ...
      this.wrapper = arguments[0].wrapper,
      this.selector = arguments[0].selector,
      this.activation = arguments[0].activation,
      this.duration = (!!arguments[0].duration && typeof arguments[0].duration !== 'undefined') ? arguments[0].duration : 400,
      this.nextContentButton = (!!arguments[0].nextContentButton && typeof arguments[0].nextContentButton !== 'undefined') ? arguments[0].nextContentButton : false,
      this.offsetArray = [],
      this.isFalse = false,
      this.isResize = false;
      ...
      function resizing() {
        ...
        _index = $(that.selector).filter('.' + that.activation).parent().index();
